...
public class SingAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        Glide.with(context).load(datas.get(2).getLives().get(position).getCover().getSrc()).into(viewHolder.ibPicture);
        viewHolder.tvContent.setText(datas.get(2).getLives().get(position).getTitle());
        viewHolder.tvName.setText(datas.get(2).getLives().get(position).getOwner().getName());
        viewHolder.tvWatchingNumber.setText(datas.get(2).getLives().get(position).getOnline()+"");
